<?php
class Bookmark_Post extends WP_REST_Controller {
  ...
  public function create_item_permissions_check( $request ) {
    ...
    if (!empty($request_body) && isset($request_body['u'])) {
        ...
        $post_id = $request_body['b'];
        $post_author = (int)get_post_field ('post_author', $post_id);

        // user not logged in
        if ($cur_user == 0) {
            return new WP_Error( 'unauthorized', 'يرجى تسجيل الدخول لتتمكن من حفظ المحتوى', array( 'status' => 401 ) );
  ...
  public function prepare_item_for_database( $request ) {
    ...
    $prepared_data['post_id'] = $request_body['b'];
    $prepared_data['action'] = $request_body['action'];
  ...
  public function create_item( $request ) {
    $bookmark_data = $this->prepare_item_for_database( $request );
    if ( !empty( $bookmark_data ) && is_array( $bookmark_data ) ) {

        if ( function_exists( 'mo_bookmark_post' ) ) {
            $result = mo_bookmark_post($bookmark_data);
