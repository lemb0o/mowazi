<?php
class Get_Tag_Controller extends WP_REST_Controller {
  ...
  public function create_item( $request ) {
    ...
    if ( function_exists( 'mo_get_tag' ) ) {

      $output_html = mo_get_tag($prepared_data);
      $page_title = $prepared_data['name'];
      $page_perma = get_tag_link( $request['tag'] );

      if ( !empty($output_html) ) {
      	$data = array( 'html'	=>	$output_html["html"], 'title'  =>  $page_title, 'perma'  =>  $page_perma );
